...
{
    class VariableNameConverterExample
    {
        private static readonly string API_KEY = Environment.GetEnvironmentVariable("APIVERVE_API_KEY") ?? "YOUR_API_KEY_HERE";
        private static readonly string API_URL = "https://api.apiverve.com/v1/variablenameconverter";

        /// <summary>
        /// Make a GET request to the Variable Name Converter API
        /// </summary>
        static async Task<JsonDocument> CallVariableNameConverterAPI()
        {
            try
            {
        ...
        {
            Console.WriteLine("📤 Calling Variable Name Converter API...\n");

            var result = await CallVariableNameConverterAPI();
